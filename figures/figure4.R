...
names(keyvals)[which(df$category == "Mitochondria gene")] <- 'Mitochondria Genes'
geneLabels <- df[which((df$category == "PD causal variant")& df$significance != 'insignificant'),]$gene_name
gba <- createVolcanoCustom(df, 'GBA+ vs Control - All', keyvals, geneLabels,c(-0.81, 0.61), c(0,18))
gba
...
ggsave('figure4.jpeg', plot = ipd  + gba + lrrk2 + snca + lrrk2_2 +p, dpi=300, height = 16, width=20)
